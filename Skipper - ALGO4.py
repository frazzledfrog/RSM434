# -*- coding: utf-8 -*-
"""
Created on Sun Feb  2 18:12:13 2025
...
ORDER_LIMIT = 10000
reset_threshold = 100000 #may not exceed 500,000 divided by four, or 125,000
...
def get_lease_tickers():
    ...
    for _ in range(5):
        if resp.ok:
            ...
            sleep(0.5)
    raise ValueError("Failed to retrieve both lease ID's after multiple attempts")
def main():
    ...
    while status == 'ACTIVE':
        ...
        if gross_position < MAX_EXPOSURE_GROSS and net_position > MAX_SHORT_EXPOSURE_NET and net_position < MAX_LONG_EXPOSURE_NET:
                ...
                # Trading behavior
                if indx_position < reset_threshold:
                ...
                if indx_position >= reset_threshold:
                    while indx_position > 0.0:
                        ...
                        if etf_position > 0.0:
                            quantity = min(reset_threshold, etf_position)
                        ...
                        # Create ETF the position
                            ...
                            quantity = min(reset_threshold, abs(etf_position))
