...
#class definition
class company:
    def __init__(self, ticker, COE, DIV_PERCENT, eps_range_mod, BID = None, ASK = None, NTM = None, LTM = None, IO = None, institute_val = None, degenerate_val = None):
        self.ticker = ticker
        self.COE = COE
        self.DIV_PERCENT = DIV_PERCENT
        self.eps_range_mod = eps_range_mod
        self.BID = BID
        self.ASK = ASK
        self.NTM = NTM
        self.LTM = LTM
        self.IO = IO
        self.institute_val = institute_val
        self.degenerate_val = degenerate_val

    def describe(self):
        description = f"Company Ticker: {self.ticker}\n"
        description += f"Cost of Equity (COE): {self.COE}%\n"
        description += f"Dividend Percentage: {self.DIV_PERCENT}%\n"

        if self.BID is not None:
            description += f"BID Price: {self.BID}\n"
        if self.ASK is not None:
            description += f"ASK Price: {self.ASK}\n"
        if self.NTM is not None:
            description += f"Next Twelve Months (NTM) Value: {self.NTM}\n"
        if self.LTM is not None:
            description += f"Last Twelve Months (LTM) Value: {self.LTM}\n"
        if self.IO is not None:
            description += f"Initial Offering (IO): {self.IO}\n"
        if self.institute_val is not None:
            description += f"Institutional Value: {self.institute_val}\n"
        if self.degenerate_val is not None:
            description += f"Degenerate Value: {self.degenerate_val}\n"

        return description #function to describe variable

    def variance(self): #function that returns the variance of the next twelve months eps
        loop = 5 - get_current_quarter()
        x = 1
        NTM_Variance = 0
        for x <= loop:
            NTM_Variance += (x * self.eps_range_mod)^2 / 3
            x += 1
        return NTM_Variance
#set up each company
TP = company("TP", 0.05, 0.8, 0.02)
AS = company("AS", 0.075, 0.5, 0.04)
BA = company("BA", 0.1, 0, 0.06)
